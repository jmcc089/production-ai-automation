...
> Source: Notion — https://app.notion.com/p/37cf9cdba745812daa3ffa8a6f81177c
> Markdown mirror of the Notion case study. Screenshots are noted inline as captions; the original images live in Notion.
...
https://ai-solutions-brief.netlify.app
...
*Note: Baseline figures are working estimates for a clinic of this size, illustrative of the problem, not audited metrics.*
...
#### 2.1.4 Architecture Decision Records (ADRs)
...
**Public intake surface.** The webhook that receives each intake is publicly reachable, because the Tally form feeding it is public by design. There is no throttling on the endpoint at this scale. A production deployment would add a shared secret between form and webhook, request throttling, and spam filtering ahead of the model call, so that abuse cannot consume model credits or pollute the queue.
...
*Note: Pain-point framing reflects working assumptions for a firm of this size — illustrative, not audited metrics.*
...
#### 2.2.4 Architecture Decision Records (ADRs)
...
*Note: Support-volume framing reflects working assumptions for a brand of this size — illustrative, not audited metrics.*
...
#### 2.3.4 Architecture Decision Records (ADRs)
...
What got built, and the proof it runs. Each case shows the live automation (the n8n workflow that's verified in the running instance), the Supabase data layer, the operator interface, and the end-to-end verification that exercised it.

System Design is the *why*; this is the *what shipped*.

### 3.1 Cedar Healthcare
...
| **Database** | `Supabase` | PostgreSQL — patients + intake queue |
...
*[Screenshot in Notion: the n8n workflow canvas for Cedar Healthcare — Intake Triage.]*

#### 3.1.3 The Data Layer (Supabase)
...
*[Screenshot in Notion: the Supabase schema for the `ch_` tables.]*

#### 3.1.4 The Interfaces

**Marketing site** — public-facing clinic site with the Tally intake form.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/cedar_healthcare/web
- Live: https://cedarhealthcare-web.up.railway.app/

**Front-desk dashboard** — staff work an urgency-sorted queue, moving cases New → In Progress → Resolved, with a per-patient timeline.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/cedar_healthcare/app
- Live: https://cedarhealthcare-app.up.railway.app/

#### 3.1.5 QA & Verification
...
| **Database** | `Supabase` | PostgreSQL — relational client → case → review |
...
*[Screenshot in Notion: the n8n workflow canvas for Holt & Vargas.]*

#### 3.2.3 The Data Layer (Supabase)
...
*[Screenshot in Notion: the Supabase schema for the `hvl_` tables.]*

#### 3.2.4 The Interfaces

**Marketing site** — public-facing firm site with the Tally intake form.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/holt_vargas/web
- Live: https://holtvargas-web.up.railway.app/

**Paralegal dashboard** — a prioritized case queue where each intake arrives pre-scored against the right checklist, with present/missing lists and a priority flag, plus completeness stats and per-case detail.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/holt_vargas/app
- Live: https://holtvargas-app.up.railway.app/

#### 3.2.5 QA & Verification
...
| **Database** | `Supabase` | PostgreSQL + Storage — catalog + tickets |
| **Email** | `Resend` | Customer reply + team notification |
| **Frontend** | Lovable / static (Railway) | Marketing site + agent console |

#### 3.3.2 The Automation (n8n)
...
n8n — Intake path (Railway)
   ...
   ▼
Agent dashboard (Railway / Lovable) ── reads/writes ──► Supabase
...
*[Screenshot in Notion: the n8n workflow canvas for Brasa Commerce.]*

#### 3.3.3 The Data Layer (Supabase)
...
*[Screenshot in Notion: the Supabase schema for the `bc_` tables.]*

**Supabase `search_products` function code:**
...
#### 3.3.4 The Interfaces

**Marketing site** — public-facing brand site with the Tally intake form.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/brasa_commerce/web
- Live: https://brasacommerce-web.up.railway.app/

**Agent dashboard** — confident routine tickets have already auto-sent, so attention goes to held tickets (complaints, low-confidence, risk-keyword). Each carries an editable draft to approve-and-send or escalate.

- Code: https://github.com/jmcc089/production-ai-automation/tree/main/brasa_commerce/app
- Live: https://brasacommerce-app.up.railway.app/
...
- GitHub: https://github.com/jmcc089/production-ai-automation
- Cedar Healthcare — site: https://cedarhealthcare-web.up.railway.app/ · dashboard: https://cedarhealthcare-app.up.railway.app/
- Holt & Vargas — site: https://holtvargas-web.up.railway.app/ · dashboard: https://holtvargas-app.up.railway.app/
- Brasa Commerce — site: https://brasacommerce-web.up.railway.app/ · dashboard: https://brasacommerce-app.up.railway.app/
